...
export default function WayDetails() {
    ...
    let cityModules = {
        'tree': {
            ...
            isLiked: treeState,
            Icon: TreeDeciduous,
            description: "Plant a tree that spends shadow and improves the air quality."
        },
        'flower-bed': {
            name: 'Flower Bed',
            likeCount: 22,
    ...
    let noMostVoted = 3;
    let mostVoted = Object.keys(cityModules).sort((a, b) => cityModules[b].likeCount - cityModules[a].likeCount).slice(0, noMostVoted);

    return (
        <div>
            <h1 className="text-2xl mb-4">Muskauer Straße</h1>
            <div>
            ...
            <div className="flex">
                <h2 className="text-xl mr-2 mb-4">Your Votes</h2>
                {
                    [...Array(5)].map((_, i) => (<Heart className="w-5 h-5 relative top-[4px]" key={'unused-heart-' + i} />))
                }
            </div>
